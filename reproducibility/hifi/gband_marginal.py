...
if __name__ == '__main__':
    ...
    cte = np.array([np.std(f[0].data[0, i, 0:200, 0:200]) / np.mean(f[0].data[0, i, 0:200, 0:200]) for i in range(f[0].data.shape[1])])

    ind = np.argsort(cte)[::-1]
    
    frames = f[0].data[:, ind[0:n_frames], :, :]
    ...
    decSI = torchmfbd.Deconvolution('gband_marginal.yaml')
    ...
    hdu2 = fits.ImageHDU(decSI.pars_s0[0].cpu().numpy())
    hdu3 = fits.ImageHDU(decSI.rho[0].cpu().numpy())
    hdul = fits.HDUList([hdu0, hdu1, hdu2, hdu3])
    hdul.writeto(f'gband_marginal.fits', overwrite=True)
